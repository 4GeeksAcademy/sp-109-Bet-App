...
export const PlaygroundEdit = () => {
    ...
    const fetchPlayground = async () => {
      try {
        const resp = await fetch(
          import.meta.env.VITE_BACKEND_URL + `/api/playground/${id}`
        );
  ...
  const Styles = () => (
    <style>{`
      ...
      .pge-scope .form-control, .pge-scope .form-select{
        border:1px solid #e8eef8; border-radius:12px; height:48px;
        box-shadow:0 8px 22px rgba(15,23,42,.06);
      }
      .pge-scope textarea.form-control{ min-height:110px; height:auto; padding-top:.75rem; }
      .pge-scope .form-control:focus, .pge-scope .form-select:focus{
        border-color:#c9dafc;
