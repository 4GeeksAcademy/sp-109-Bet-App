...
export const Requests = () => {
  ...
  // 2. Cargar solicitudes solo si hay usuario
  useEffect(() => {
  ...
  const handleAction = async (reqId, action) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/requests/${reqId}/${action}`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  if (user === undefined) return <div className="container mt-5">Cargando...</div>;
        ...
        <ul className="list-group mb-4">
          {received.map((req) => (
            <li key={req.id} className="list-group-item d-flex justify-content-between align-items-center">
              <span>
                <strong>{req.user_name}</strong> quiere unirse a <strong>{req.playground_name}</strong>
              </span>
              <div>
